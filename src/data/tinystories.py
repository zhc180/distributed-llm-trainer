...
class TinyStoriesIterableDataset(IterableDataset):
    """Streaming TinyStories dataset with optional tokenization cache."""

    def __init__(self, cfg: TinyStoriesConfig, rank: int = 0, world_size: int = 1):
        self.cfg = cfg
        self.rank = rank
...
def create_tinystories_dataloader(
    ...
    if streaming:
        dataset = TinyStoriesIterableDataset(cfg, rank=rank, world_size=world_size)
        sampler = None
        shuffle = False
    else:
